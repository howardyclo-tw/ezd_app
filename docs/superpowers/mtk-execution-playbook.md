...
7. **指定驗證範圍(編排者填,實作者不可自選)**:tsc + 改動檔 lint(不得新增違規)+ 指定的 spec 清單。**「禁止自跑全套 e2e(審查者獨占跑,避免資源競爭)」**。dev server:`http://[::1]:3000`,沒起就背景 `pnpm dev`。
8. Commit 訊息指定;結尾要求「結構化報告+完整指令輸出;誠實回報失敗」。

## REVIEW prompt 必備條款

1. 「你沒寫這段程式;以實際 diff 與『親自跑測試』驗證,勿信任實作者報告」+ 內嵌實作報告。
2. 逐項核對 brief 的功能點與守衛(列出來)。
3. **MUTATION TEST(必做)**:挑 1–2 個核心守衛/邏輯,暫時註解→重跑對應測試→必須變紅→`git checkout --` 還原。仍綠=假綠燈=NEEDS_FIXES。
4. **獨占跑全套**:`pnpm exec playwright test e2e/regression e2e/features`(5R.8 後含 `e2e/journeys`);疑似 timeout flake 單獨重跑該 spec 一次並註明。回報實際數字。
5. tsc + unit + 改動檔 lint;稽核測試範圍是否足夠(「哪個該跑而沒跑的 spec?」)。
6. UI task 加查樣式一致性(§0.5)。
